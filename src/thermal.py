...
class BitmapHeaderInfo:
    ...
    def __init__(self, bytes):
        ...
        if int.from_bytes(bytes[32:36], 'little') > 2:
            raise ValueError # we accept at most 2 colors?
        if int.from_bytes(bytes[36:40], 'little') > 2:
            raise ValueError # we accept at most 2 significant colors?
...
class ThermalPrinter:
    ...
    def __init__(self, bus=1, baudrate=9600, pins=None, **kwargs):
        ...
        extra_options = {}
        if pins is not None:
            extra_options['tx'] = pins[0]
            extra_options['rx'] = pins[1]

        self.uart = UART(bus, baudrate)
        self.uart.init(stop=1, **extra_options)
    ...
    def printBarcode(self, text, style):
